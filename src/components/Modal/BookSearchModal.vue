      ...
      <div v-show="activeTab === 'upload'" class="tab-content">
        ...
        >
          ...
          <div v-else class="drop-zone-content">
            ...
            <p class="drop-text">Drop your EPUB file here</p>
            <p class="drop-hint">or click to browse</p>
            <p class="drop-format">Only .epub files are supported</p>
          </div>
        </div>
...
const emit = defineEmits(['close', 'upload', 'download'])
...
// Focus search input when modal opens or search tab is activated
...
async function handleSearch() {
  ...
  try {
    ...
    error.value = err.message || 'Failed to search for books'
    hasSearched.value = true
  } finally {
...
async function processFile(file) {
  ...
  if (!file.name.toLowerCase().endsWith('.epub')) {
    uploadError.value = 'Please select an EPUB file (.epub)'
    return
  }
  ...
  try {
    ...
    uploadError.value = err.message || 'Failed to upload book'
    isUploading.value = false
    uploadProgress.value = 0
  }
}
...
async function handleDownload(book) {
  ...
  try {
    // Download the book file using fast download API
    const arrayBuffer = await fastDownloadBook(book.detailUrl, (progress) => {
    ...
    // Emit download event with book data
    emit('download', {
      title: book.title,
      author: book.author,
      coverUrl: coverDataUrl,
    ...
    downloadError.value = err.message || 'Failed to download book'
    downloadingBookId.value = null
    downloadProgress.value = 0
  }
}
