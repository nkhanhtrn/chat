    ...
    <div class="setting-item setting-item-vertical">
      ...
      <div v-if="firestoreInfo" class="debug-info-grid">
        ...
        <template v-else>
          ...
          <span class="debug-info-val">
            <button v-if="!cloudStorageInfo && !cloudStorageLoading" class="debug-fetch-btn" @click="refreshCloudStorage">Fetch</button>
            <template v-else-if="cloudStorageLoading">Loading…</template>
            <template v-else>{{ cloudStorageInfo!.error ? '—' : formatBytes(cloudStorageInfo!.totalBytes) }} ({{ cloudStorageInfo!.fileCount }} files)</template>
          </span>
...
const { logBuffer, isCapturing, startCapture, stopCapture, clearLogs, storageInfo, browserInfo, firestoreInfo, cloudStorageInfo, cloudStorageLoading, refresh, refreshCloudStorage, ensureLoaded } = useDebugInfo()
...
.debug-fetch-btn { padding: 0.15rem 0.6rem; background: var(--color-bg-elevated); border: 1px solid var(--color-border-base); border-radius: 4px; cursor: pointer; font-size: 0.75rem; font-family: system-ui, -apple-system, sans-serif; color: var(--color-text-muted); transition: all 0.15s; }
.debug-fetch-btn:hover { background: var(--color-bg-hover); color: var(--color-text-base); }
