...
const onBlur = (event: FocusEvent) => {
  if (wrapperRef.value?.contains(event.relatedTarget as Node)) return
  cancelEditing()
}
...
.inline-edit-input:focus { border-color: var(--color-primary); box-shadow: 0 0 0 2px var(--color-primary-subtle, rgba(99, 102, 241, 0.1)); }
.inline-edit-buttons { position: absolute; right: 4px; top: 50%; transform: translateY(-50%); display: flex; align-items: center; gap: 2px; }
.inline-edit-btn { background: none; border: none; padding: 3px; cursor: pointer; color: var(--color-text-muted); opacity: 0.7; display: flex; align-items: center; justify-content: center; border-radius: 3px; transition: opacity 0.15s ease; }
