...
describe('BookStorage', () => {
  ...
  describe('saveCoverImage', () => {
    it('saves cover data to IndexedDB', async () => {
      const coverData = new ArrayBuffer(4)
      await BookStorage.saveCoverImage('book-123', coverData)

      expect(mockDb.put).toHaveBeenCalledWith('book-covers', coverData, 'book-123')
    })

    it('warns if book-covers store is not available', async () => {
      mockDb.objectStoreNames.contains.mockReturnValue(false)
      const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {})

      await BookStorage.saveCoverImage('book-123', new ArrayBuffer(4))

      expect(mockDb.put).not.toHaveBeenCalled()
      expect(warnSpy).toHaveBeenCalledWith(
        expect.stringContaining('book-covers store not available'),
      )
      warnSpy.mockRestore()
    })
  })
  ...
  describe('getCoverImage', () => {
    it('returns cover data when found', async () => {
      const coverData = new ArrayBuffer(6)
      mockDb.get.mockResolvedValue(coverData)

      const result = await BookStorage.getCoverImage('book-123')

      expect(result).toBe(coverData)
    })
    ...
    it('returns null if book-covers store is not available', async () => {
      mockDb.objectStoreNames.contains.mockReturnValue(false)
  ...
  describe('deleteCoverImage', () => {
    it('deletes cover from IndexedDB', async () => {
      await BookStorage.deleteCoverImage('book-123')

      expect(mockDb.delete).toHaveBeenCalledWith('book-covers', 'book-123')
    })
